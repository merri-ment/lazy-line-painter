...
function () {
  ...
  _createClass(LazyLinePainter, [{
    ...
    value: function paint() {
      ...
      this._paint(); // fire onStart callback
    ...
    value: function _calcPathDurations() {
      var startTime =
      /* this.config.reverse ? this.config.totalDuration : */
      this.config.delay;

      for (var _i5 = 0; _i5 < this.config.paths.length; _i5++) {
        var path = this.config.paths[_i5];
        path.progress = 0; // if (this.config.reverse) {
    ...
    key: "_getProgress",
    value: function _getProgress(linear_progress, ease) {
      var progress = linear_progress;

      if (ease) {
        progress = _easing.default[ease](linear_progress);
      }
